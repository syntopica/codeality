...
describe('explainDoBlock', () => {
  it('opens a cursor over the statement as a dollar-quoted literal, rolls it back, and keeps the plan in a session setting', () => {
    const sql = "select 'it''s', $$a;b$$"
    const block = explainDoBlock(sql)
    const match =
      /^do \$(dbq_[0-9a-f]{16})\$ declare c refcursor; p text; begin begin open c for execute 'explain \(analyze, buffers, format json\) ' \|\| \$(dbq_[0-9a-f]{16})\$(.*)\$\2\$; fetch c into p; close c; raise exception using errcode = 'DBQRB'; exception when sqlstate 'DBQRB' then null; end; perform set_config\('dbq\.plan', p, false\); end \$\1\$$/s.exec(
        block,
      )
