...
from codeality_py.units.declaration_for import declaration_for
from codeality_py.units.top_level_statements import top_level_statements
...
def collect_declarations(tree: ast.Module, relative_path: str) -> tuple[Declaration, ...]:
    ...
    declarations = (
        declaration_for(statement, relative_path) for statement in top_level_statements(tree)
    )
