...
from codeality_py.suppression.build_suppression import build_suppression
from codeality_py.suppression.suppression import Suppression

_INLINE = re.compile(r"#\s*codeality-py:\s*ignore\[(?P<code>[A-Z]{3}\d{3})\]")
_FILE_WIDE = re.compile(
    r"#\s*codeality-py:\s*ignore-file\[(?P<code>[A-Z]{3}\d{3})\](?:\s*reason:\s*(?P<reason>.+))?"
)
