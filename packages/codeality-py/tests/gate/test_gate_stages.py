...
from codeality_py.gate.gate_stages import gate_stages
...
def test_a_recorded_baseline_is_honored_by_the_structural_stage(
    ...
    (tmp_path / ".codeality-py-baseline.json").write_text("{}", encoding="utf-8")
    config = BaselineConfig(project_root=tmp_path)
    assert _commands(config)["codeality-py"] == ("codeality-py", "baseline", "check")


def test_without_a_baseline_the_structural_stage_runs_check(tmp_path: Path) -> None:
    config = BaselineConfig(project_root=tmp_path)
    assert _commands(config)["codeality-py"] == ("codeality-py", "check")
