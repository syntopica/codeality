...
- Move to **ESLint 9+** and **flat config** before layering
  `@vibracomet/eslint-config`. This may be the largest step; treat it as its own
  milestone.
...
1. Add a new `eslint.config.ts` that imports from `@vibracomet/eslint-config`
...
Switch `prettier.config.*` to `@vibracomet/prettier-config` variants. Update
`tsconfig.json` to extend `@vibracomet/tsconfig/*` and resolve duplicate
compiler options.
