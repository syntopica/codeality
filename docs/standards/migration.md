...
1. Install `prettier`, `@syntopica/prettier-config`, and the peer plugins for
...
2. Replace any existing `.prettierrc*` or `prettier.config.*` with the standard
   config that imports from `@syntopica/prettier-config`.
...
2. Extend `@syntopica/eslint-config/base` plus the appropriate framework
   extension.
...
1. Add `"extends": "@syntopica/tsconfig/<variant>.json"` to `tsconfig.json`.
